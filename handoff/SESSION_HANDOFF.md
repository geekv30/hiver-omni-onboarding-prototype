...
8. **Backdrop rotated (Geeky's call, taken).** The clip is 720x1280 portrait; as a landscape full-bleed it upscaled 2.0x using only 35% of the frame — the whole reason it read soft, with no higher-res master in the repo. Now a quarter turn: 1.25x using 90%. Scoped to a `.learning-video-bg--bleed` modifier, because `learning-card.html` renders the same video class inside an `inset:40px` card where `100dvh/100vw` would size against the wrong box. Marks strengthened with layered text-shadows (tight pass for edge definition + wide halo); scrim untouched per rule 3.

9. **Two of my own regressions, caught by Geeky and fixed.** (a) The widget reveal was gated on a sessionStorage "seen" flag, so *every* visit after the first in a tab snapped — including a plain reload, which is how Geeky reviews a screen. Now keyed off `performance.getEntriesByType("navigation")[0].type`: fresh visit and reload play in full, only `back_forward` snaps. learning.html had the same latent bug (a reload gave the compressed chain) — reloads now always run full length. (b) The layered text-shadows produced a visible dark slab behind each label: `.learning-step__label-text` needs `overflow:hidden` for its ellipsis, which **clips a wide shadow into a hard-edged rectangle the width of the span**. Reverted to the original single soft pass.

**Two rules earned the hard way this session:**
- **Never gate a reveal on "have they seen it", gate it on "how did they arrive."** A session flag cannot tell a reload from a return, and reload is the review path.
- **Wide `text-shadow` passes are unusable on any element with `overflow: hidden`** — they merge across adjacent glyphs and get clipped into a slab. Tight passes (<=2px) only.

**Open / next — the one real item:** **learning.html's marks do not meet WCAG over the moving backdrop, and did not before this session either.** Measured on composited pixels at six points across the clip, worst case, against a 3:1 bar for the 24px/500 heading: **classic 2.78:1, rotated 2.18:1**. Text-shadow tuning is a **dead end** — it buys ~0.1 because a glyph-hugging shadow cannot change the broad field, and going wider produces the slab above. It reads acceptably by eye, so this is a compliance gap, not a visible break. The rule-3 answer is already in the standing rules: **dark marks on a `mix-blend-mode: multiply` layer**, which read over both the white field and the blue fold. That turns the white type dark and changes the screen's character, so put it to Geeky rather than shipping it. Do **not** reach for the scrim.
