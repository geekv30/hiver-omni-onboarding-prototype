...
**Open / next — the one real item:** **learning.html's marks do not meet WCAG over the moving backdrop, and did not before this session either.** Measured on composited pixels at six points across the clip, worst case, against a 3:1 bar for the 24px/500 heading: **classic 2.78:1, rotated 2.18:1, and text-shadow tuning moves it only ~0.1** because a glyph-hugging shadow cannot change the broad field. It reads acceptably by eye at the brightest frame, so this is a compliance gap rather than a visible break. The rule-3 answer is the one already written into the standing rules: **dark marks on a `mix-blend-mode: multiply` layer**, which read over both the white field and the blue fold. That is a real visual change to the screen's character (white type becomes dark), so it was not done unilaterally at the end of a session — put it to Geeky. Do **not** reach for the scrim.
